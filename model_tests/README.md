...
This workspace member (`model_tests`) is a dedicated integration testing suite designed to validate the real-world performance of LLMs against the Auwgent sandbox engines.

Instead of unit-testing the Rust engine with hardcoded strings, this suite makes live API calls to LLMs (via Groq), forces them to write Lua or JavaScript code to solve specific scenarios, executes that code in the sandbox, and validates the architectural outcome (e.g., verifying parallel batching, data dependencies, and correct data output).
...
# See the exact script the model generated for each scenario
cargo run --release -p model_tests -- --verbose

# Run the same live scenarios against the QuickJS sandbox
...
| **`parallel`** | Parallel Tool Yield. | Model correctly batches two independent tool calls into a single Lua `await_all()` tuple or JS `Promise.all()` batch, proving Auwgent's N+1 prevention works. |
...
* 🟢 **PASS:** The model wrote syntactically valid sandbox code, obeyed the engine's tool-await contract, and structurally solved the scenario.
